function nn = saeff_s(nn, x, y,beta,L_A_S, alfa_S, L_O_S,phi_S)
...
%% add pairwise constraints on connected nodes J_2 %%
H=nn.a{2}(:,2:end); %hidden representation learned from autoencoder

if (alfa_S~=0)
    nn.L= nn.L+ (alfa_S/m)*trace(H'*L_A_S*H);
end

%% add pairwise constraints on common labeled nodes J_3 %%
if (phi_S~=0)
   nn.L= nn.L+(phi_S/m)*trace(H'*L_O_S*H);
end
