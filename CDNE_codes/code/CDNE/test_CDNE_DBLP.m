...
%% An Example Case for cross-network node classification in citation networks %%
clear all;
...
r_T=40; %weight of conditional MMD
...
%% deep network embedding in source network
O_S=(Y_S)*(Y_S)'; % whether two nodes have common labels
O_S(find(O_S==0))=-1; %if the nodes i and j are with different labels, O_S(i,j)=-1;
...
for trpindex=1:length(trp) %index in trp
    ...
    for randomSplit=1:numSplit
        ...
        %% Deep network embedding in target network
